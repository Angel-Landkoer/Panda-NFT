  ...
  return (
    <>
      <section className="cartWidget">
        <button className="cartToggle" onClick={handleToggle}>Cart</button>
        <div className={toggle ? "contentActive" : "contentHide"}>
          {cartProducts.map((item, i) => {
            return (
              <div key={`context${i}`} className="context">
                <img className="contextImg" src={item.images.original.url} alt="" />
                <p className="contextTitle">Title: {item.title}</p>
                <p className="contextDescription">Description: {item.user.description}</p>
              </div>
            );
          })}
